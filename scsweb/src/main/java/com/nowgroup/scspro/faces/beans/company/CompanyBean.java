...
public class CompanyBean {
	...
	public String remove(CompanyModel item) {
		try {
			companyServiceImpl.deleteCompany(deModelize(item));
		} catch (Exception e) {
			log.error(e.getMessage(), e);
			FacesContext.getCurrentInstance().addMessage(
					null,
					new FacesMessage(FacesMessage.SEVERITY_ERROR,
							companyMsgBundle.getString("company.remove.error"),
							e.getMessage()));
		}
	...
	public String uploadCompany() {
		...
		try {
			if (getCompany().getId() != 0) {
				...
				getCompanyServiceImpl().updateCompany(deModelize(company));
			} else {
				log.debug("Adding new record");
				log.debug("roles in record: "
						+ company.getCompanyScope().size());
				getCompanyServiceImpl().addCompany(deModelize(company));
			}
			...
			FacesContext.getCurrentInstance().addMessage(
					null,
					new FacesMessage(FacesMessage.SEVERITY_ERROR,
							companyMsgBundle.getString("company.save.error"), e
									.getMessage()));
			return "";
		}
	}

	private Company deModelize(CompanyModel model) {
		Company result = new Company();
		result.setId(model.getId());
		result.setName(model.getName());
		result.setAlias(model.getAlias());

		result.setAddressStreet(model.getAddressStreet());
		result.setAddressAdditional(model.getAddressAdditional());
		result.setCity(model.getCity());
		result.setState(model.getState());
		result.setZip(model.getZip());

		result.setPhone(model.getPhone());
		result.setPhoneSecondary(model.getPhoneSecondary());
		result.setFax(model.getFax());
		result.setEmail(model.getEmail());
		result.setWeb(model.getWeb());

		result.setCompanyScope(model.getCompanyScope());
	...
	public List<CompanyScopeModel> getRolesList() {
		...
		if (this.getCompany().getId() != 0) {
			...
			List<CompanyScope> scopes = getCompanyServiceImpl()
					.getCompanyScope(this.getCompany().getId());

			for (CompanyScope scope : scopes) {
				String translate = getCompanyScopeServiceImpl()
	...
	public List<CompanyRoleModel> getAvailableRolesList() {
		...
		if (this.getCompany().getId() == 0) {
			...
			for (CompanyScopeModel usedScope : getRolesList()) {
				CompanyRole usedRole = getCompanyScopeServiceImpl()
						.getRoleInCompanyScope(usedScope.getId());
		...
		for (CompanyRole role : allRoles) {
			...
			crm.setDisplayName(companyMsgBundle.getString("companies.profiles."
	...
	public void onRemoveRole(DragDropEvent event) {
		...
		if (availableRoles != null) {
			availableRoles.invokeOnComponent(FacesContext.getCurrentInstance(),
					event.getDragId(), new ContextCallback() {
						...
						public void invokeContextCallback(FacesContext context,
							...
							CompanyScopeModel item = draggedItem != null ? (CompanyScopeModel) draggedItem
									.getAttributes().get("companyRole")
									: new CompanyScopeModel();

							try {
								removeRole(item);
								FacesMessage msg = new FacesMessage(
										FacesMessage.SEVERITY_ERROR,
										"Role removed from company: " + item,
										null);
								FacesContext.getCurrentInstance().addMessage(
										null, msg);
							} catch (Exception e) {
								...
								FacesMessage msg = new FacesMessage(
										...
										"Could not remove role: " + item, null);
								FacesContext.getCurrentInstance().addMessage(
										null, msg);
							}
						}
			...
			FacesMessage msg = new FacesMessage(FacesMessage.SEVERITY_ERROR,
					"Available roles not found onAddRole context", null);
			...
			availableRoles.invokeOnComponent(FacesContext.getCurrentInstance(),
					event.getDragId(), new ContextCallback() {
						...
						public void invokeContextCallback(FacesContext context,
								UIComponent target) {
							HtmlPanelGroup draggedItem = (HtmlPanelGroup) target;
							CompanyRoleModel item = draggedItem != null ? (CompanyRoleModel) draggedItem
									.getAttributes().get("companyRole")
									: new CompanyRoleModel();

							try {
								addRole(item);
								FacesMessage msg = new FacesMessage(
										FacesMessage.SEVERITY_ERROR,
										"Role added to company: " + item, null);
								FacesContext.getCurrentInstance().addMessage(
										null, msg);
							} catch (Exception e) {
								log.error(e.getMessage(), e);
	...
	public void setCompanyRoleServiceImpl(
			CompanyRoleService companyRoleServiceImpl) {
