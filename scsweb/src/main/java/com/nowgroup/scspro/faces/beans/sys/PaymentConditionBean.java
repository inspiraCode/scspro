...
public class PaymentConditionBean implements Serializable {
	...
	try {
	    paymentConditionServiceImpl.deletePaymentCondition(paymentCondition);
	} catch (Exception e) {
	...
	try {
		...
		getPaymentConditionServiceImpl().updatePaymentCondition(paymentCondition);
	    else
		getPaymentConditionServiceImpl().addPaymentCondition(paymentCondition);
	    return "success";
	} catch (Exception e) {
	    log.error(e.getMessage(), e);
	    FacesContext.getCurrentInstance().addMessage(null,
