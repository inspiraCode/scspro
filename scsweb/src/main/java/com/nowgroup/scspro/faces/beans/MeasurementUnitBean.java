...
import com.nowgroup.scspro.model.MeasurementUnitModel;
import com.nowgroup.scspro.model.MeasurementUnitRoleModel;
...
public class MeasurementUnitBean implements Serializable {
	...
	private static Logger log = Logger.getLogger(MeasurementUnitBean.class
			.getName());
	...
	@ManagedProperty("#{measurementUnitServiceImpl}")
	private MeasurementUnitService measurementUnitServiceImpl;

	@ManagedProperty("#{measurementUnitRoleServiceImpl}")
	...
	public List<MeasurementUnitModel> getMeasurementUnitList() {
		if (measurementUnitBundle == null) {
			...
			setMeasurementUnitBundle(context.getApplication()
					.getResourceBundle(context, "i18n_measurement_unit"));
		...
		List<MeasurementUnit> source = measurementUnitServiceImpl
				.getMeasurementUnits();

		for (MeasurementUnit mu : source) {
			MeasurementUnitModel mum = new MeasurementUnitModel(mu);
			List<MeasurementUnitRole> murList = measurementUnitServiceImpl
					.getMeasurementUnitRoles(mu.getId());
			String displayRoles = "";
			for (MeasurementUnitRole mur : murList) {
				displayRoles += ", "
						+ measurementUnitBundle
								.getString("measurement-unit.profiles."
										+ mur.getName().toLowerCase());
			}
	...
	public String remove(MeasurementUnit item) {
		try {
			...
			getMeasurementUnitServiceImpl().deleteMeasurementUnit(clone);
		} catch (Exception e) {
			log.error(e.getMessage(), e);
			String errorMessage = measurementUnitBundle
					.getString("measurement-unit.remove.error");
	...
	public String uploadMeasurementUnit() {
		try {
			if (getMeasurementUnit().getId() != 0) {
				getMeasurementUnitServiceImpl().updateMeasurementUnit(
						measurementUnit);
			} else {
				...
				getMeasurementUnitServiceImpl().addMeasurementUnit(
						measurementUnit);
			}
			return "success";
		} catch (Exception e) {
			log.error(e.getMessage(), e);
			String errorMessage = measurementUnitBundle
					.getString("measurement-unit.save.error");
			FacesMessage fm = new FacesMessage(FacesMessage.SEVERITY_ERROR,
					errorMessage, e.getMessage());
			FacesContext.getCurrentInstance().addMessage(null, fm);
			return "";
	...
	public void onRemoveRole(DragDropEvent event) {
		HtmlPanelGroup availableRoles = (HtmlPanelGroup) event.getComponent()
				.findComponent("selectedRoles");
		if (availableRoles != null) {
			availableRoles.invokeOnComponent(FacesContext.getCurrentInstance(),
					event.getDragId(), new ContextCallback() {

						public void invokeContextCallback(FacesContext context,
								UIComponent target) {
							HtmlPanelGroup draggedItem = (HtmlPanelGroup) target;
							MeasurementUnitRoleModel item = draggedItem != null ? (MeasurementUnitRoleModel) draggedItem
									.getAttributes().get("measurementRole")
									: new MeasurementUnitRoleModel();
							try {
								log.debug("selected role for removal:" + item);
								removeRole(item);
								String errorMessage = measurementUnitBundle
										.getString("measurement-unit.role.remove.success");
								...
								log.error(e.getMessage(), e);
								String errorMessage = measurementUnitBundle
										.getString("measurement-unit.role.remove.error");
								FacesMessage fm = new FacesMessage(
			...
			String errorMsg = measurementUnitBundle
					.getString("measurement-unit.role.remove.error");
	...
	public void onAddRole(DragDropEvent event) {
		...
		if (availableRoles != null) {
					...
					event.getDragId(), new ContextCallback() {
						...
						public void invokeContextCallback(FacesContext context,
							...
							try {
								...
								addRole(item);
								String errorMessage = measurementUnitBundle
										.getString("measurement-unit.role.add.success");
								FacesMessage fm = new FacesMessage(
										FacesMessage.SEVERITY_INFO,
										errorMessage, null);
								FacesContext.getCurrentInstance().addMessage(
								...
								log.error(e.getMessage(), e);
								String errorMessage = measurementUnitBundle
										.getString("measurement-unit.role.add.error");
								FacesMessage fm = new FacesMessage(
										FacesMessage.SEVERITY_ERROR,
										errorMessage, e.getMessage());
								FacesContext.getCurrentInstance().addMessage(
			...
			String errorMsg = measurementUnitBundle
					.getString("measurement-unit.add.remove.error");
			FacesMessage msg = new FacesMessage(FacesMessage.SEVERITY_ERROR,
					errorMsg, null);
			FacesContext.getCurrentInstance().addMessage(null, msg);
		}
	...
	public List<MeasurementUnitRoleModel> getRolesList() {
		...
		if (this.getMeasurementUnit().getId() != 0) {
			rolesList = new ArrayList<MeasurementUnitRoleModel>();
			List<MeasurementUnitRole> mur = getMeasurementUnitServiceImpl()
					.getMeasurementUnitRoles(this.getMeasurementUnit().getId());
			...
			for (MeasurementUnitRole role : mur) {
				...
				murm.setDisplayName(measurementUnitBundle
						.getString("measurement-unit.profiles."
	...
	public List<MeasurementUnitRoleModel> getAvailableRoles() {
		...
		List<MeasurementUnitRole> allRoles = measurementUnitRoleServiceImpl
				.getMeasurementUnitRoles();
		...
		for (MeasurementUnitRole role : allRoles) {
			...
			String translated = measurementUnitBundle
					.getString("measurement-unit.profiles." + murmCode);
	...
	public void setMeasurementUnitServiceImpl(
			MeasurementUnitService measurementUnitServiceImpl) {
	...
	public void setMeasurementUnitRoleServiceImpl(
			MeasurementUnitRoleService measurementUnitRoleServiceImpl) {
	...
	private void removeMeasurementUnitRole(MeasurementUnitRole item) {
		measurementUnitServiceImpl.removeMeasurementUnitRole(this
				.getMeasurementUnit().getId(), item);
		rolesList.remove(item);
	}
	...
	public void addMeasurementUnitRole(MeasurementUnitRole item) {
		measurementUnitServiceImpl.addMeasurementUnitRole(this
				.getMeasurementUnit().getId(), item);
	}
