...
public class PaymentConditionBean implements Serializable {
	...
	private static final Logger log = Logger
			.getLogger(PaymentConditionBean.class.getName());

	private PaymentCondition paymentCondition = new PaymentCondition();

	@ManagedProperty("#{paymentConditionServiceImpl}")
	...
	public List<PaymentCondition> getPaymentConditions() {
		return paymentConditionServiceImpl.getPaymentCondition();
	}
		...
		try {
			...
			FacesContext.getCurrentInstance().addMessage(
					...
					new FacesMessage(FacesMessage.SEVERITY_ERROR,
							paymentCndBundle.getString("pay_cnd.remove.error"),
							e.getMessage()));
		}
		return "pay-cnd-list";
	}
	...
	public String uploadPaymentCondition() {
		try {
			if (paymentCondition.getId() != 0)
				getPaymentConditionServiceImpl().updatePaymentCondition(
						paymentCondition);
			else
				getPaymentConditionServiceImpl().addPaymentCondition(
						paymentCondition);
			return "success";
		} catch (Exception e) {
			log.error(e.getMessage(), e);
			FacesContext.getCurrentInstance().addMessage(
					null,
					new FacesMessage(FacesMessage.SEVERITY_ERROR,
							paymentCndBundle.getString("pay_cnd.save.error"), e
									.getMessage()));
		}
		return "";
	}
	...
	public void setPaymentConditionServiceImpl(
			PaymentConditionService paymentConditionServiceImpl) {
